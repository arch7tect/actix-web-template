...
**Problem**: Logs don't appear in Loki or Grafana Explore

**Solution 1**: Install Loki Docker driver plugin

```bash
docker plugin install grafana/loki-docker-driver:2.9.3 --alias loki --grant-all-permissions
docker plugin ls  # Should show loki plugin
```

**Solution 2**: Recreate containers with logging driver

```bash
docker compose up -d --force-recreate app
```

**Solution 3**: Verify logging configuration

```bash
# Check app container is using Loki driver
docker inspect memos-app | jq '.[0].HostConfig.LogConfig'
# Should show: {"Type": "loki", ...}
```

**Solution 4**: Check time range in Grafana

- Set time range to "Last 5 minutes" in Grafana Explore
- Try query: `{container_name="memos-app"}`

### Loki "too many outstanding requests"

**Problem**: Loki query fails with "too many outstanding requests"

**Solution 1**: Restart Loki

```bash
docker compose restart loki
sleep 15  # Wait for Loki to be ready
```

**Solution 2**: Reduce query scope

- Use shorter time ranges ("Last 5 minutes" instead of "Last 1 hour")
- Set lower line limits (100 instead of 1000) in Options
- Add filters to narrow results: `{container_name="memos-app"} != "health"`

### No Traces in Jaeger

**Problem**: No traces appear in Jaeger UI

**Solution 1**: Verify OTLP endpoint

```bash
docker compose exec app env | grep OTLP
# Should show: OTLP_ENDPOINT=http://jaeger:4317
```

**Solution 2**: Check Jaeger is receiving traces

```bash
curl http://localhost:16686/api/services | jq
# Should show services including your app
```
...
for i in {1..5}; do curl -s http://localhost:3737/api/v1/memos > /dev/null; done
```

### Grafana Can't Connect to Datasources

**Problem**: Grafana shows datasource errors

**Solution 1**: Verify datasource provisioning
...
**Solution 3**: Check datasources in Grafana

```bash
curl -s http://localhost:3001/api/datasources -u admin:admin | jq
# Should show Prometheus, Loki, and Jaeger
```
...
**Solution 1**: Verify Prometheus config

```bash
ls observability/prometheus.yml
# File must exist with correct scrape config
```

**Solution 2**: Check Prometheus targets

```bash
curl -s http://localhost:9090/api/v1/targets | jq '.data.activeTargets[] | {job: .labels.job, health: .health}'
# App target should show "up"
```

**Solution 3**: Verify app exposes metrics
